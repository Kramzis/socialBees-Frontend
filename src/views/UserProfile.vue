...
export default {
  ...
  methods: {
    getUserInfo() {
      ...
      axios.get(`http://localhost:8081/user/${userId}`, config)
          .then(response => {
            ...
            this.checkIfUserFollowing(this.userInfo.id)
          }).catch(error => {
        console.log(error)
      })
    },
    ...
    checkIfUserFollowing(followingId) {
      ...
      axios.get(`http://localhost:8081/follow/${userId}/${followingId}`, config)
          .then(response => {
    ...
    unfollow(followingId) {
      ...
      const decodeData = decodeToken(localStorage.getItem("token"))
      let userId = decodeData.userId
      axios.delete(`http://localhost:8081/follow/unfollow/${userId}/${followingId}`, config)
          .then(response => {
